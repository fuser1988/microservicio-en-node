...
describe('Add, remove and filter data', () => {
  ...
  it('should add a track to an album', () => {
    const artist = createAndAddArtist(unqfy, 'Guns n\' Roses', 'USA');
    const album = createAndAddAlbum(unqfy, artist.id, 'Appetite for Destruction', 1987);
    const track = createAndAddTrack(unqfy, album.id, 'Welcome to the jungle', 200, new arrayListMod.ArrayList('rock', 'hard rock'));
  ...
  it('should find different things by name', () => {
    ...
    const track = createAndAddTrack(unqfy, album1.id, 'Roses track', 200, new arrayListMod.ArrayList('pop', 'movie'));
    const playlist = unqfy.createPlaylist('Roses playlist', new arrayListMod.ArrayList('pop'), 1400);
    const results = unqfy.searchByName('Roses');
    assert.deepEqual(results, {
      artists: new arrayListMod.ArrayList(artist1),
      albums: new arrayListMod.ArrayList(album1),
      tracks: new arrayListMod.ArrayList(track),
      playlists: new arrayListMod.ArrayList(playlist),
    });
  });
  ...
  it('should get all tracks matching genres', () => {
    ...
    const t0 = createAndAddTrack(unqfy, album1.id, 'Welcome to the jungle', 200, new arrayListMod.ArrayList('rock', 'hard rock', 'movie'));
    const t1 = createAndAddTrack(unqfy, album1.id, 'Sweet Child o\' Mine', 500, new arrayListMod.ArrayList('rock', 'hard rock', 'pop', 'movie'));

    const artist2 = createAndAddArtist(unqfy, 'Michael Jackson', 'USA');
    const album2 = createAndAddAlbum(unqfy, artist2.id, 'Thriller', 1987);
    const t2 = createAndAddTrack(unqfy, album2.id, 'Trhiller', 200, new arrayListMod.ArrayList('pop', 'movie'));
    createAndAddTrack(unqfy, album2.id, 'Another song', 500, new arrayListMod.ArrayList('classic'));
    const t3 = createAndAddTrack(unqfy, album2.id, 'Another song II', 500, new arrayListMod.ArrayList('movie'));

    const tracksMatching = unqfy.getTracksMatchingGenres(new arrayListMod.ArrayList('pop', 'movie'));
  ...
  it('should get all tracks matching artist', () => {
    const artist = createAndAddArtist(unqfy, 'Guns n\' Roses', 'USA');
    const album = createAndAddAlbum(unqfy, artist.id, 'Appetite for Destruction', 1987);
    const t1 = createAndAddTrack(unqfy, album.id, 'Welcome to the jungle', 200, new arrayListMod.ArrayList('rock', 'hard rock'));
    const t2 = createAndAddTrack(unqfy, album.id, 'It\'s so easy', 200, new arrayListMod.ArrayList('rock', 'hard rock'));

    const album2 = createAndAddAlbum(unqfy, artist.id, 'Use Your Illusion I', 1992);
    const t3 = createAndAddTrack(unqfy, album2.id, 'Don\'t Cry', 500, new arrayListMod.ArrayList('rock', 'hard rock'));

    const artist2 = createAndAddArtist(unqfy, 'Michael Jackson', 'USA');
    const album3 = createAndAddAlbum(unqfy, artist2.id, 'Thriller', 1987);
    createAndAddTrack(unqfy, album3.id, 'Thriller', 200, new arrayListMod.ArrayList('pop', 'movie'));
    createAndAddTrack(unqfy, album3.id, 'Another song', 500, new arrayListMod.ArrayList('classic'));
    createAndAddTrack(unqfy, album3.id, 'Another song II', 500, new arrayListMod.ArrayList('movie'));
...
describe('Playlist Creation and properties', () => {
  ...
  it('should create a playlist as requested', () => {
    const artist = createAndAddArtist(unqfy, 'Guns n\' Roses', 'USA');
    const album = createAndAddAlbum(unqfy, artist.id, 'Appetite for Destruction', 1987);
    const t1 = createAndAddTrack(unqfy, album.id, 'Welcome to the jungle', 200, new arrayListMod.ArrayList('rock', 'hard rock', 'movie'));
    createAndAddTrack(unqfy, album.id, 'Sweet Child o\' Mine', 1500, new arrayListMod.ArrayList('rock', 'hard rock', 'pop', 'movie'));

    const artist2 = createAndAddArtist(unqfy, 'Michael Jackson', 'USA');
    const album2 = createAndAddAlbum(unqfy, artist2.id, 'Thriller', 1987);
    const t2 = createAndAddTrack(unqfy, album2.id, 'Thriller', 200, new arrayListMod.ArrayList('pop', 'movie'));
    const t3 = createAndAddTrack(unqfy, album2.id, 'Another song', 500, new arrayListMod.ArrayList('pop'));
    const t4 = createAndAddTrack(unqfy, album2.id, 'Another song II', 500, new arrayListMod.ArrayList('pop'));

    const playlist = unqfy.createPlaylist('my playlist', new arrayListMod.ArrayList('pop', 'rock'), 1400);
